...
SELECT COUNT(name) AS facility_count
FROM `Facilities`
WHERE membercost = 0
...
SELECT surname,
       ...
       joindate
FROM `Members`
WHERE DATE(joindate) = '2012-09-26'

/* Q7: How can you produce a list of all members who have used a tennis court?
Include in your output the name of the court, and the name of the member
formatted as a single column. Ensure no duplicate data, and order by
the member name. */
SELECT DISTINCT fac.name, CONCAT(mem.surname, ', ', mem.firstname)
FROM `Members` mem
...
ORDER BY CONCAT(mem.surname, ', ', mem.firstname)
...
FROM `Members` mem
JOIN `Bookings` book ON mem.memid = book.memid AND mem.memid > 0
...
SELECT fac.name, (SUM(book.slots * fac.guestcost) - fac.monthlymaintenance) AS revenue
FROM `Bookings` book
JOIN `Facilities` fac ON book.facid = fac.facid AND book.memid = 0
GROUP BY fac.name
HAVING revenue < 1000
...
--------------------------------------------------------------------------------------
-- count ubique id
SELECT COUNT(DISTINCT bookid) AS bookidcount,
 	   COUNT(DISTINCT facid) AS facidcount,
 	   COUNT(DISTINCT memid) AS memidcount
FROM `Bookings`
